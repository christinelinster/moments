...
- A running PostgreSQL instance reachable through `DATABASE_URL`, plus a dedicated `_test` or `_e2e` database for Playwright.
...
- `createdb moments`: create the development database once, if it does not exist.
- `createdb moments_test`: create the dedicated Playwright database once, if it does not exist.
...
- `E2E_DATABASE_URL=postgresql://postgres:postgres@localhost:5432/moments_test npm run test:e2e`: run the isolated browser suite after creating the dedicated database. The command creates and drops the `moments_e2e` schema and uses a temporary media directory. It refuses to run against a database whose name does not end in `_test` or `_e2e`.
